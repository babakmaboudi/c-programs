...
void Model::export_vtk()
{
	ofstream file;
	char path[50];
	sprintf(path,"./output/oxygen%d.vtk",vtk_count);
	file.open(path);
	...
	sprintf(path,"./output/hydrogen%d.vtk",vtk_count);
	file.open(path);
	...
	file << "DATASET POLYDATA" << endl;
	file << endl;
	file << "POINTS " << 2*num_mols << " float" << endl;

	for(int i=0 ; i<num_mols ; i++)
	{
		Vec temp = (all_sites[i])[2].r;
		file << temp.x() << " " << temp.y() << " " << temp.z() << endl;
		temp = (all_sites[i])[3].r;
		file << temp.x() << " " << temp.y() << " " << temp.z() << endl;
	}
	file.close();	
	vtk_count++;
