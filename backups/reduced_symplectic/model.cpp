...
void Model::deriv_symp_reduced(Matrix* dy, double t, Matrix* y, Parameters* param,int indicator)
{
	...
	Matrix z = A * (*y);
	double coef = - param->mu / pow( pow(z.at(0),2) + pow(z.at(1),2) + pow(z.at(2),2) , 1.5 );
...
void Model::deriv_symp_reduced(Matrix* dy, double t, Matrix* y, Parameters* param,int indicator)
{
	...
	{
		Matrix orig = A * (*y);
		double coef = - param->mu / pow( pow(orig.at(0),2) + pow(orig.at(1),2) + pow(orig.at(2),2) , 1.5 );
		Matrix f;
		f.zeros(6,1);
		f.at(3) = coef * orig.at(0);
		f.at(4) = coef * orig.at(1);
		f.at(5) = coef * orig.at(2);
		(*dy) = A.tr() * f;
		cout << (*dy) << endl;
	}
}
...
*/
	...
	phi = U.get_submat(0,U.get_num_rows(),0,2);
...
void Model::test_reduced_model()
{
	...
	Matrix init_red = A.tr() * init_cond;
